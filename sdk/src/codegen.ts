...
export function generate(witText: string): string {
  ...
  out.push("    private session: KontorSession,");
  out.push("    private address: ContractAddress,");
  out.push("  ) {}");
  out.push("");
  for (const [name, { function: fn }] of methods) {
    ...
    const encodeLine = `    const expr = _wit.encodeCall(${nameLit}, JSON.stringify({ ${wireArgFields} }));`;

    if (isViewFn(fn, ctx)) {
      // view-context → read-only RPC, resolves directly to the value.
      out.push(`  async ${safeName}(${sig}): Promise<${resultTs}> {`);
      out.push(encodeLine);
      out.push("    const resp = await this.session.view(this.address, expr);");
      if (fn.result == null) {
        out.push("    return;");
      } else {
        out.push(
          `    const raw = JSON.parse(_wit.decodeResult(${nameLit}, resp));`,
        );
        out.push(`    return ${decodeExpr(fn.result, "raw", ctx)};`);
      }
      out.push("  }");
    } else {
      // proc-context → returns an Inst, executed when awaited/submitted.
      out.push(`  ${safeName}(${sig}): Inst<${resultTs}> {`);
      out.push(encodeLine);
      if (fn.result == null) {
        out.push(
          `    return this.session.call(this.address, ${nameLit}, expr, () => {});`,
        );
      } else {
        out.push(
          `    return this.session.call(this.address, ${nameLit}, expr, (resp: string) => {`,
        );
        out.push(
          `      const raw = JSON.parse(_wit.decodeResult(${nameLit}, resp));`,
        );
