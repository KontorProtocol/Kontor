...
/**
 * True when `fn`'s first parameter is a `view-context` borrow — those
 * become read-only `Promise`-returning methods. Everything else
 * (proc-context, core-context, or no ctx param) is treated as a proc
 * and returns an `Inst`.
 */
function isViewFn(fn: WitFunction, ctx: Ctx): boolean {
  const ctxParam = fn.params[0]?.type;
  if (typeof ctxParam !== "number") return false;
  const k = ctx.kinds[ctxParam];
  if (k.tag !== "handle") return false;
...
/** True when `ref` resolves to a `result<_, _>` WIT type. */
function isResultRef(ref: TypeRef | null, ctx: Ctx): boolean {
 ...
 * named `vout`, both return a `result`, and `detach`'s params are a
 * name-subset of `attach`'s (minus `vout`). When matched, codegen emits
 * a single `attachment(...)` builder and suppresses the bare `attach` /
 * `detach` — a lone attach orphans the asset and a lone detach can't
 * execute, so neither is safe to expose standalone.
 */
function findAttachPair(
  methods: Method[],
  ctx: Ctx,
): { attach: WitFunction; detach: WitFunction } | null {
  ...
  if (!detach.params.slice(1).every((p) => attachNames.has(p.name))) {
    return null;
  }
...
/**
 * Emit the `attachment(...)` builder for a recognized attach/detach
 * pair. Its params are `attach`'s (minus the builder-owned `vout`,
 * which compose pins to output 0); it builds both the attach and
 * detach `Inst`s and returns an `Attachment` that owns the round-trip.
 */
function emitAttachment(
  ...
  const attachWire = ['"vout": "0"']
    ...
    .join(", ");
  const detachWire = detach.params
    .slice(1)
    .map(
      (p) =>
        `${JSON.stringify(p.name)}: ${encodeExpr(p.type, toCamel(p.name), ctx)}`,
  ...
  return [
    `  attachment(${sig}): Attachment<${resultTs}> {`,
...
export function generate(witText: string): string {
  ...
  const methods: Method[] = Object.entries(root.exports).filter(
    (e): e is Method => "function" in e[1],
  );
  const attachPair = findAttachPair(methods, ctx);
  ...
  // Emit the Contract class. Behavior lives in `ContractBase`; the
  // generated methods are typed one-liners delegating to it.
  out.push("export class Contract extends ContractBase {");
  out.push("  constructor(session: KontorSession, address: ContractAddress) {");
  out.push("    super(session, address, _wit);");
  out.push("  }");
  out.push("");
  for (const [name, { function: fn }] of methods) {
    // The attach/detach pair collapses into one `attachment(...)`
    // builder; the bare `attach` / `detach` are suppressed.
    if (attachPair != null && (name === "attach" || name === "detach")) {
      if (name === "attach") {
        out.push(...emitAttachment(attachPair.attach, attachPair.detach, ctx));
        out.push("");
      }
      continue;
    }
