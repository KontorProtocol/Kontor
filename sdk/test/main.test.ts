...
test("codegen Tier 1: emits TS types + Contract interface from token.wit", () => {
  ...
  expect(out).toContain("export class Contract extends ContractBase {");

  // proc method: drops ctx, returns an Inst
  expect(out).toMatch(/  transfer\(dst: HolderRef, amt: Decimal\): Inst</);

  // view method: drops ctx, return type (option<decimal>) flattened to T | null
  expect(out).toMatch(/  balance\(acc: HolderRef\): Promise<Decimal \| null>/);
...
test("codegen Tier 2: emits Contract class bound to a session + per-type helpers", () => {
  ...
  // Contract class extends ContractBase, threading its embedded `_wit`.
  expect(out).toContain("export class Contract extends ContractBase {");
  expect(out).toContain(
    "constructor(session: KontorSession, address: ContractAddress) {",
  );
  expect(out).toContain("super(session, address, _wit);");

  // view-context methods delegate to `_view`; proc-context methods to
  // `_proc` (which returns an Inst). Behavior lives in ContractBase.
  expect(out).toMatch(
    /\n  balance\([^)]*\): Promise<[\s\S]*?this\._view\([\s\S]*?"balance"/,
  );
  expect(out).toMatch(
    /\n  transfer\([^)]*\): Inst<[\s\S]*?this\._proc\([\s\S]*?"transfer"/,
  );
...
test("codegen Tier 2: attach/detach pair collapses into an attachment() builder", () => {
  const out = generate(tokenWit);
  // The paired entrypoint is emitted; the bare attach/detach are
  // suppressed — a lone attach orphans the asset, a lone detach can't
  // execute, so neither is exposed standalone.
