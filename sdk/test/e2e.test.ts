/**
 * End-to-end test of the codegen pipeline. The token contract's TS
 * bindings are pre-generated by `vitest.globalSetup.ts` into
 * `__generated__/token.ts` so this test runs identically in Node and
 * browser environments. Catches structural bugs in the round-trip:
 *   canonical class → .toRaw() → WAVE encode → mock chain → WAVE
 *   decode → <Class>.fromRaw(raw) → typed value
 */
import { test, expect } from "vitest";
import { type KontorTransport, Decimal, HolderRef } from "@kontor/sdk";
