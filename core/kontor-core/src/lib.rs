//! Pure-Rust core of the Kontor SDK: `Inst` (de)serialization, BLS key
//! material + signing, the WIT/WAVE codec, and WIT validation. This crate
//! has no `.cargo/config.toml` target pin and no wit-bindgen dependency, so
//! it compiles both to `wasm32-unknown-unknown` (via the `kontor-sdk`
//! wrapper) and to native / mobile targets (via the `kontor-mobile` uniffi
//! wrapper). The bodies here are the single source of truth for these
