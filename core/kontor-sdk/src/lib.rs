...
impl Guest for Lib {
    ...
    fn aggregate_signing_message(
        ...
        kontor_core::aggregate_signing_message(claim_json, nonce, sponsored, inst_json)
    }

    fn bls_aggregate_signatures(signatures: Vec<Vec<u8>>) -> Result<Vec<u8>, String> {
        kontor_core::bls_aggregate_signatures(signatures)
    }

    fn validate_wit(wit_content: String) -> ValidationResult {
        match kontor_core::validate_wit(wit_content) {
            kontor_core::WitValidation::Ok => ValidationResult::Ok,
            kontor_core::WitValidation::ParseError(msg) => ValidationResult::ParseError(msg),
            kontor_core::WitValidation::ValidationErrors(errors) => {
