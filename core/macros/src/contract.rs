...
pub fn generate(config: Config) -> TokenStream {
    ...
    let (resource_types, records_with_resources, with_clause) = match parse_wit_resources(&wit_path, &world) {
        Ok((res, recs, with)) => (res, recs, with),
        Err(e) => {
            // Fallback to original behavior if parsing fails
            eprintln!("Warning: Failed to parse WIT resources: {}", e);
            (HashSet::new(), HashSet::new(), TokenStream::new())
        }
    };
    ...
    quote! {
        ...
        // Import types for use in the contract
        use kontor::built_in::context;
        use kontor::built_in::error;
        use kontor::built_in::foreign;
        use kontor::built_in::numbers;

        fn make_keys_iterator<T: FromString>(keys: kontor::built_in::context::Keys) -> impl Iterator<Item = T> {
            struct KeysIterator<T: FromString> {
                keys: kontor::built_in::context::Keys,
        ...
        impl ReadContext for kontor::built_in::context::ViewContext {
        ...
        #[automatically_derived]
        impl ReadContext for kontor::built_in::context::ProcContext {
            fn __get_str(&self, path: &str) -> Option<String> {
                self.get_str(path)
            }
        ...
        #[automatically_derived]
        impl WriteContext for kontor::built_in::context::ProcContext {
        ...
        impl stdlib::Store for numbers::Integer {
            fn __set(ctx: &impl stdlib::WriteContext, path: stdlib::DotPathBuf, value: Self) {
                ...
                let s = built_in::numbers::integer_to_string(value);
                ctx.__set_str(&path, &s);
            }
        }
        
        impl stdlib::Retrieve for numbers::Integer {
            fn __get(ctx: &impl stdlib::ReadContext, path: stdlib::DotPathBuf) -> Option<Self> {
                // Convert from string representation
                ctx.__get_str(&path).map(|s| built_in::numbers::string_to_integer(&s))
            }
        }
        
        impl Default for numbers::Integer {
            fn default() -> Self {
                built_in::numbers::u64_to_integer(0)
            }
        }
        ...
        impl stdlib::Retrieve for foreign::ContractAddress {
            fn __get(ctx: &impl stdlib::ReadContext, path: stdlib::DotPathBuf) -> Option<Self> {
