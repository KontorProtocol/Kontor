...
mod footprint_tests {
    ...
    fn folds_rows_into_per_contract_totals() {
        ...
        assert_eq!(c1.deposit.parse::<f64>().unwrap(), 15.0);
        assert_eq!(c1.footprint_bytes, 150);
        let c2 = by.iter().find(|c| c.contract_id == 2).unwrap();
        assert_eq!(c2.deposit.parse::<f64>().unwrap(), 30.0);
