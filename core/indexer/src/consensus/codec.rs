...
use crate::consensus::vote::{decode_votetype, encode_votetype};
use crate::consensus::{Address, Ctx, Height, Proposal, ProposalPart, Value, ValueId, Vote};
...
impl Codec<SignedConsensusMsg<Ctx>> for ProtobufCodec {
    type Error = ProtoError;

    fn decode(&self, bytes: Bytes) -> Result<SignedConsensusMsg<Ctx>, Self::Error> {
    ...
    fn encode(&self, msg: &SignedConsensusMsg<Ctx>) -> Result<Bytes, Self::Error> {
        match msg {
...
) -> Result<RoundCertificate<Ctx>, ProtoError> {
    Ok(RoundCertificate {
...
impl Codec<LivenessMsg<Ctx>> for ProtobufCodec {
    type Error = ProtoError;

    fn decode(&self, bytes: Bytes) -> Result<LivenessMsg<Ctx>, Self::Error> {
    ...
    fn encode(&self, msg: &LivenessMsg<Ctx>) -> Result<Bytes, Self::Error> {
        match msg {
...
impl Codec<ProposedValue<Ctx>> for ProtobufCodec {
    type Error = ProtoError;

    fn decode(&self, bytes: Bytes) -> Result<ProposedValue<Ctx>, Self::Error> {
...
impl Codec<sync::Status<Ctx>> for ProtobufCodec {
    type Error = ProtoError;

    fn decode(&self, bytes: Bytes) -> Result<sync::Status<Ctx>, Self::Error> {
    ...
    fn encode(&self, msg: &sync::Status<Ctx>) -> Result<Bytes, Self::Error> {
...
impl Codec<sync::Request<Ctx>> for ProtobufCodec {
    type Error = ProtoError;

    fn decode(&self, bytes: Bytes) -> Result<sync::Request<Ctx>, Self::Error> {
    ...
    fn encode(&self, msg: &sync::Request<Ctx>) -> Result<Bytes, Self::Error> {
        let proto = match msg {
...
impl Codec<sync::Response<Ctx>> for ProtobufCodec {
    type Error = ProtoError;

    fn decode(&self, bytes: Bytes) -> Result<sync::Response<Ctx>, Self::Error> {
        decode_sync_response(proto::SyncResponse::decode(bytes)?)
    }

    fn encode(&self, response: &sync::Response<Ctx>) -> Result<Bytes, Self::Error> {
        encode_sync_response(response).map(|proto| proto.encode_to_vec().into())
    }
}

impl HasEncodedLen<sync::Response<Ctx>> for ProtobufCodec {
    fn encoded_len(
        &self,
        response: &sync::Response<Ctx>,
    ) -> Result<usize, <Self as Codec<sync::Response<Ctx>>>::Error> {
...
) -> Result<sync::Response<Ctx>, ProtoError> {
...
pub fn encode_synced_value(
    synced_value: &sync::RawDecidedValue<Ctx>,
...
) -> Result<sync::RawDecidedValue<Ctx>, ProtoError> {
...
pub(crate) fn decode_polka_certificate(
    ...
    Ok(PolkaCertificate {
        ...
        polka_signatures: certificate
            .signatures
            .into_iter()
            .map(|sig| -> Result<PolkaSignature<Ctx>, ProtoError> {
                let address = sig.validator_address.ok_or_else(|| {
...
pub fn decode_extension(ext: proto::Extension) -> Result<SignedExtension<Ctx>, ProtoError> {
    let signature = ext
        .signature
...
pub fn encode_extension(ext: &SignedExtension<Ctx>) -> Result<proto::Extension, ProtoError> {
    Ok(proto::Extension {
...
pub fn encode_vote(vote: &SignedVote<Ctx>) -> Result<proto::SignedMessage, ProtoError> {
...
pub fn decode_vote(msg: proto::SignedMessage) -> Result<SignedVote<Ctx>, ProtoError> {
    let signature = msg
        .signature
        .ok_or_else(|| ProtoError::missing_field::<proto::SignedMessage>("signature"))?;
