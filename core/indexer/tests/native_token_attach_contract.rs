...
use indexer_types::{
    ComposeQuery, Inst, InstructionEnvelope, InstructionQuery, RevealParticipantQuery, RevealQuery,
    serialize,
};
use testlib::*;
    ...
    let query = ComposeQuery::builder()
        .instructions(vec![
            InstructionQuery::builder()
                ...
                .instruction_envelope(InstructionEnvelope::single(attach_inst.clone()))
                .chained_instruction_envelope(InstructionEnvelope::single(detach_inst.clone()))
                .build(),
        ])
        .sat_per_vbyte(2)
        .envelope(600)
