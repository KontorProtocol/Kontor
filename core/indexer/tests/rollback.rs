...
use indexer_types::{
    AggregateInst, Event, Inst, InstructionEnvelope, ParsedInput, Signer, SignerRef, Transaction,
};
...
async fn test_reactor_rollback_reverts_registration_state() -> Result<()> {
    ...
    let block1 = indexer_types::Block {
        ...
        transactions: vec![Transaction {
            ...
            index: 0,
            inputs: vec![parsed_input(
                0,
                Signer::XOnlyPubKey(x_only_public_key.to_string()),
                InstructionEnvelope::single(Inst::RegisterBlsKey {
...
/// Rollback of a BLS key registration performed inside an aggregate envelope.
///
...
/// aggregate-envelope path in block_handler, which first runs aggregate
/// signature verification and then calls
...
/// again from scratch).
#[tokio::test]
async fn test_reactor_rollback_reverts_aggregate_registration() -> Result<()> {
    let cancel_token = CancellationToken::new();
    let (_reader, writer, _temp_dir) = new_test_db().await?;
    let conn = &writer.connection();
    ...
    let register_op = AggregateInst {
        signer: SignerRef::XOnlyPubKey(x_only_public_key.to_string()),
    ...
    assert!(
        state_count > 0,
        "aggregate registration must write contract_state at height 1"
    );
    ...
    assert_eq!(
        ...
        "rollback must remove contract_state from aggregate registration"
    );
    ...
    assert!(
        state_reinserted > 0,
        "re-inserted aggregate registration must recreate contract_state (proves full revert)"
    );
