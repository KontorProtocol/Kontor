...
async fn setup_tokens(runtime: &Runtime, user: &str) -> Result<()> {
    ...
    runtime
        .execute(Some(user), &token_a_addr(), "init()")
        .await?;
    runtime
        .execute(Some(user), &token_b_addr(), "init()")
        .await?;
    ...
    runtime
        .execute(
            Some(user),
            &token_a_addr(),
            "mint({r0: 100000, r1: 0, r2: 0, r3: 0, sign: plus})",
        )
        .await?;
    runtime
        .execute(
            Some(user),
            &token_b_addr(),
            "mint({r0: 200000, r1: 0, r2: 0, r3: 0, sign: plus})",
        )
        .await?;
...
async fn create_pool_manually(
    ...
    let balance_a_result = runtime
        .execute(
            Some(user),
            &token_a_addr(),
            &format!(
                "withdraw({{r0: {}, r1: 0, r2: 0, r3: 0, sign: plus}})",
                amount_a
            ),
        )
        .await?;

    let balance_b_result = runtime
        .execute(
            Some(user),
            &token_b_addr(),
            &format!(
    ...
    Ok(format!(
        "Pool would be created with {} token A and {} token B",
        amount_a, amount_b
    ))
}
...
async fn test_amm_setup() -> Result<()> {
    ...
    let alice_balance_a = runtime
        .execute(None, &token_a_addr(), "balance(\"alice\")")
        .await?;
    let alice_balance_b = runtime
        .execute(None, &token_b_addr(), "balance(\"alice\")")
        .await?;
...
async fn test_pool_creation_concept() -> Result<()> {
    ...
    let balance_a = runtime
        .execute(None, &token_a_addr(), "balance(\"alice\")")
        .await?;
    let balance_b = runtime
        .execute(None, &token_b_addr(), "balance(\"alice\")")
        .await?;
...
async fn test_cross_contract_deposit_effect() -> Result<()> {
    ...
    let alice_initial = runtime
        .execute(None, &token_a_addr(), "balance(\"alice\")")
        .await?;
    let bob_initial = runtime
        .execute(None, &token_a_addr(), "balance(\"bob\")")
        .await?;
    ...
    let alice_after_withdraw = runtime
        .execute(None, &token_a_addr(), "balance(\"alice\")")
        .await?;
