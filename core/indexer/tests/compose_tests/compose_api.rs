...
use indexer_types::{
    ComposeQuery, ContractAddress, Inst, InstructionQuery, Insts, OpReturnData, RevealInputs,
...
pub async fn test_compose(reg_tester: &mut RegTester) -> Result<()> {
    ...
    let derived_token_data = serialize(&Insts {
        ops: vec![instruction.clone()],
        aggregate: None,
    })?;
...
pub async fn test_compose_all_fields(reg_tester: &mut RegTester) -> Result<()> {
    ...
    let derived_chained_tap_script = Builder::new()
        ...
        .push_slice(b"kon")
        .push_opcode(OP_0)
        .push_slice(PushBytesBuf::try_from(serialize(&Insts {
            ops: vec![derived_chained_instruction.clone()],
            aggregate: None,
        })?)?)
        .push_opcode(OP_ENDIF)
        .into_script();
...
pub async fn test_compose_param_bounds_and_fee_rate(reg_tester: &mut RegTester) -> Result<()> {
    ...
    let query2 = ComposeQuery::builder()
        .instructions(vec![InstructionQuery {
            ...
            chained_insts: Some(Insts {
                ops: vec![chained_oversized_inst],
                aggregate: None,
            }),
        }])
        .sat_per_vbyte(2)
