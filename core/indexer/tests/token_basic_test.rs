...
async fn test_token_withdraw_deposit() -> Result<()> {
    ...
    runtime.execute(Some(alice), &token_addr, "mint({r0: 1000, r1: 0, r2: 0, r3: 0, sign: plus})").await?;
    
    // Check balance
    let balance = runtime.execute(None, &token_addr, &format!("balance(\"{}\")", alice)).await?;
    assert!(balance.contains("1000"));
    
    // Withdraw creates a resource (returns ok with a handle)
    let result = runtime.execute(Some(alice), &token_addr, "withdraw({r0: 100, r1: 0, r2: 0, r3: 0, sign: plus})").await?;
...
async fn test_token_basic_operations() -> Result<()> {
    ...
    runtime.execute(Some(alice), &token_addr, &format!("transfer(\"{}\", {{r0: 250, r1: 0, r2: 0, r3: 0, sign: plus}})", bob)).await?;
    
    // Check balances
    let alice_balance = runtime.execute(None, &token_addr, &format!("balance(\"{}\")", alice)).await?;
    assert!(alice_balance.contains("750"));
    
    let bob_balance = runtime.execute(None, &token_addr, &format!("balance(\"{}\")", bob)).await?;
