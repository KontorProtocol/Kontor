...
async fn test_complete_defi_resource_flow() -> Result<()> {
    ...
    runtime
        .execute(
            Some(alice),
            &usdc_addr(),
            "mint({r0: 1000000, r1: 0, r2: 0, r3: 0, sign: plus})",
        )
        .await?; // 1M USDC
    runtime
        .execute(
            Some(alice),
            &weth_addr(),
            "mint({r0: 500, r1: 0, r2: 0, r3: 0, sign: plus})",
        )
        .await?; // 500 WETH
    ...
    runtime
        .execute(
            Some(bob),
            &usdc_addr(),
            "mint({r0: 10000, r1: 0, r2: 0, r3: 0, sign: plus})",
        )
        .await?; // 10k USDC
    ...
    runtime
        .execute(
            Some(charlie),
            &usdc_addr(),
            "mint({r0: 50000, r1: 0, r2: 0, r3: 0, sign: plus})",
        )
        .await?; // 50k USDC
    runtime
        .execute(
            Some(charlie),
            &weth_addr(),
            "mint({r0: 25, r1: 0, r2: 0, r3: 0, sign: plus})",
        )
        .await?; // 25 WETH
    ...
    let alice_usdc_balance = runtime
        .execute(
            Some(alice),
            &usdc_addr(),
            "withdraw({r0: 100000, r1: 0, r2: 0, r3: 0, sign: plus})",
        )
        .await?;
    let alice_weth_balance = runtime
        .execute(
            Some(alice),
            &weth_addr(),
            "withdraw({r0: 50, r1: 0, r2: 0, r3: 0, sign: plus})",
        )
        .await?;

    println!(
        "✅ Alice withdrew Balance resources: USDC={}, WETH={}",
        alice_usdc_balance.len(),
        alice_weth_balance.len()
    );

    // Verify Alice's ledger balances were reduced (proving resources were actually moved)
    let alice_usdc_remaining = runtime
        .execute(None, &usdc_addr(), "balance(\"alice\")")
        .await?;
    let alice_weth_remaining = runtime
        .execute(None, &weth_addr(), "balance(\"alice\")")
        .await?;
    ...
    let bob_usdc_initial = runtime
        .execute(None, &usdc_addr(), "balance(\"bob\")")
        .await?;
    assert!(bob_usdc_initial.contains("10000"));

    // Bob withdraws USDC as a Balance resource for trading
    let bob_trade_balance = runtime
        .execute(
            Some(bob),
            &usdc_addr(),
    ...
    println!(
        "✅ Bob withdrew {} USDC as Balance resource for trading",
        if bob_trade_balance.contains("Error") {
            "ERROR"
        } else {
            "1000"
        }
    ...
    let charlie_usdc = runtime
        .execute(
            Some(charlie),
            &usdc_addr(),
            "withdraw({r0: 5000, r1: 0, r2: 0, r3: 0, sign: plus})",
        )
        .await?;

    println!(
        "✅ Charlie withdrew Balance resource: {}",
        if charlie_usdc.contains("Error") {
            "ERROR"
        } else {
            "SUCCESS"
        }
    ...
    let charlie_remaining = runtime
        .execute(None, &usdc_addr(), "balance(\"charlie\")")
        .await?;
    ...
    let _charlie_balance_for_alice = runtime
        .execute(
            Some(charlie),
            &usdc_addr(),
            "withdraw({r0: 1000, r1: 0, r2: 0, r3: 0, sign: plus})",
    ...
    let total_supply = runtime
        .execute(None, &usdc_addr(), "total-supply()")
        .await?;
    println!("💰 Total USDC supply: {}", total_supply);

    // Verify all balances add up correctly
    let alice_final = runtime
        .execute(None, &usdc_addr(), "balance(\"alice\")")
        .await?;
    let bob_final = runtime
        .execute(None, &usdc_addr(), "balance(\"bob\")")
        .await?;

    println!("📊 Final balances:");
    println!(
        "   Alice: {} USDC (started with 1M, withdrew 100k)",
        alice_final.trim()
    );
    println!(
        "   Bob: {} USDC (started with 10k, withdrew 1k)",
        bob_final.trim()
    );
    println!(
        "   Charlie: {} USDC (started with 50k, withdrew 6k)",
        charlie_final.trim()
    );
