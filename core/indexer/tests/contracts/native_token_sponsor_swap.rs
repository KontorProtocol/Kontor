...
async fn test_native_token_sponsor_swap() -> Result<()> {
    ...
    let seller_reveal_psbt =
        bitcoin::Psbt::deserialize(&hex::decode(&seller_compose.reveal.psbt_hex)?)?;
    ...
    let buyer_reveal_psbt =
        bitcoin::Psbt::deserialize(&hex::decode(&buyer_compose.reveal.psbt_hex)?)?;
