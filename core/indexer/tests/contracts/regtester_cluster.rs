...
async fn cluster_consensus_lifecycle() -> Result<()> {
    ...
    // A healthy node asked to stop must exit 0 — the other half of "a dead
    // subsystem exits non-zero". Reporting failure on every shutdown would be as
    // useless as reporting success on every one: an orchestrator that sees a
    // failure on ordinary rollouts learns to ignore the signal. This is the
    // regression a careless supervisor introduces, and the cluster is already up
    // here, so it costs one signal.
