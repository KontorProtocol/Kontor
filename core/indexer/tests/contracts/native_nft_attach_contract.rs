...
async fn test_native_nft_attach_contract() -> Result<()> {
    ...
    assert_eq!(
        nft::count_nfts_by_holder(runtime, HolderRef::SignerId(seller_signer_id)).await?,
        1
    );
    let seller_held_after_mint =
    ...
    // Holder index follows the UTXO: attach moves the NFT out of the seller's
    // bucket (count 0, empty list) and into the UTXO holder's bucket (count 1).
    ...
    // Holder index follows the NFT back: detach returns it to the seller's
    // bucket (count 1) and empties the UTXO holder's bucket (count 0).
    assert_eq!(
        nft::count_nfts_by_holder(runtime, HolderRef::SignerId(seller_signer_id)).await?,
        1
    );
    let seller_held_after_detach =
        nft::list_nfts_by_holder(runtime, HolderRef::SignerId(seller_signer_id), 0, 100).await?;
    ...
    assert_eq!(
        nft::list_nfts_by_holder(runtime, utxo_holder_ref, 0, 100).await?,
        Vec::<nft::NftInfo>::new()
    );
