...
use indexer_types::{ContractAddress, Inst};
...
fn bls_bulk_aggregate_signature_fails_if_op_bytes_change() {
    ...
    let msg1_mutated = op1_mutated.aggregate_signing_message(1).unwrap();
...
fn bls_bulk_call_roundtrip_serialization_preserves_signer_id() {
    ...
    let op = call_op(7, 50_000, contract, "eval(10, id)");

    let bytes = indexer_types::serialize(&(42u64, op.clone())).expect("serialize");
    let decoded: (u64, Inst) = indexer_types::deserialize(&bytes).expect("deserialize");
...
fn bls_bulk_message_changes_when_signer_id_changes() {
    ...
    let op1 = call_op(0, 50_000, contract.clone(), "eval(10, id)");
    let op2 = call_op(0, 50_000, contract, "eval(10, id)");
...
fn bls_bulk_message_changes_when_nonce_changes() {
    ...
    let op1 = call_op(0, 50_000, contract.clone(), "eval(10, id)");
    let op2 = call_op(1, 50_000, contract, "eval(10, id)");

    let msg1 = op1.aggregate_signing_message(1).unwrap();
    let msg2 = op2.aggregate_signing_message(1).unwrap();
...
fn bls_bulk_message_changes_when_gas_limit_changes() {
    ...
    let op1 = call_op(0, 50_000, contract.clone(), "eval(10, id)");
    let op2 = call_op(0, 60_000, contract, "eval(10, id)");
...
fn bls_bulk_message_changes_when_expr_changes() {
    ...
    let op1 = call_op(0, 50_000, contract.clone(), "transfer(\"alice\", 10)");
    let op2 = call_op(0, 50_000, contract, "transfer(\"bob\", 10)");
...
fn bls_bulk_wrong_signer_key_fails_single_op() {
    ...
    let msg = op.aggregate_signing_message(1).unwrap();
...
fn bls_bulk_wrong_signer_key_fails_multi_op_key_swap() {
    ...
    let op_a = call_op(0, 50_000, contract.clone(), "transfer(\"x\", 10)");
    let op_b = call_op(0, 50_000, contract, "transfer(\"y\", 20)");
    let msg_a = op_a.aggregate_signing_message(1).unwrap();
    let msg_b = op_b.aggregate_signing_message(2).unwrap();
...
proptest! {
    ...
    fn signing_message_no_panic_on_arbitrary_call(
        ...
        let msg = op.aggregate_signing_message(signer_id).expect("signing_message must not fail");
