...
async fn test_fib_contract() -> Result<()> {
    ...
    // Submit 1: fib(8) + set proxy → fib (independent mutations, unordered)
    let mut submit = runtime.submit();
    let h_fib = submit.push(&signer, fib::fib_call(&fib, arith.clone(), 8));
    submit.push(
        &signer,
    ...
    // fib(8) through proxy (depends on proxy pointing to fib)
    ...
    // Submit 2: fib_of_sub(18,10) + fib_of_sub(10,18) (independent, unordered)
    let mut submit = runtime.submit();
    let h1 = submit.push(
        &signer,
    ...
    // Reentrancy prevented (expected failure, separate)
