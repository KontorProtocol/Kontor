...
//!
//! Storage-deposit cost: each index entry is an ordinary `set-void` write (one
//! `contract_state` row), so it carries its own storage deposit (the index path's
//! bytes × D) just like the value row — there is no unmetered side channel. A
//! value with N `#[index]` fields therefore counts the value's deposit plus one per
//! live index entry toward its setter's storage-deposit floor; a hot,
//! frequently-rewritten indexed field is the costliest shape. This is intentional
//! (index rows are real, collateralized state), but authors
//! should expect indexed maps to cost proportionally more deposit than plain ones.
//!
