...
// the live subset. `where_<index>(value)` lookups are typed: the method name is
// the index and the argument is the bucket field's real type (no stringly-typed
// name or key), generated by the value's `Indexed` derive. A *sorted* index's
// `where_` returns a `SortedScan` with `.up_to(bound)` / `.range(lo..=hi)`, so
// `expire` walks only the due prefix of the active bucket instead of every active
// challenge. For the WIT records (`agreement-data`/`challenge-data`) the index
// declarations + `#[derive(Indexed)]` are injected by the `indexed = "..."` arg on
// `contract!` (forked wit-bindgen); internal structs (`NodeState`) carry `#[index]`
// directly.
//
...
//
// Deferred: partial/`filter` indexes, covering indexes (avoid the where_*→get N
// reads), and flattening `agreement_nodes` to a tuple-keyed `memberships`
// IndexedMap. See the upgrade backlog. (The `due` sorted index now covers the
// composite `(status, deadline_height)` sweep `expire` needed.)
// ─────────────────────────────────────────────────────────────────
...
// `contract!`, which injects the struct-level `#[index(...)]` declarations +
