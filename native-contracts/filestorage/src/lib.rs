...
// name or key), generated by the value's `Indexed` derive. A *composite* index
// takes one argument per bucket field — an `Option` field by its presence
// (`Presence::Absent`/`Present`), so "active ∧ unchallenged" is one prefix scan,
// not a scan-and-filter. A *sorted* index's `where_` returns a `SortedScan` with
// `.up_to(bound)` / `.range(lo..=hi)`, so `expire` walks only the due prefix of
// the active bucket instead of every active challenge. For the WIT records
// (`agreement-data`/`challenge-data`) the index declarations + `#[derive(Indexed)]`
