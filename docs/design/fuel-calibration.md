...
## Host-call coverage regressions

Starting from main `f64b9ab3` (#570), five focused tests reproduce free missing
reads, terminal cursor polls, context/transaction/floor accessors and malformed
path validation. A zero-budget read also accesses the resource table before
rejecting for fuel. The cursor test instruments the stream to distinguish charging
before polling from charging after work. These tests fail before implementation;
they exercise the real generated host traits without depending on specific prices.
